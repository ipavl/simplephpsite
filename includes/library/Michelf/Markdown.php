...
class Markdown implements MarkdownInterface {
	...
	protected function hashHTMLBlocks($text) {
		...
		# Regular expression for the pages of a block tag.
		$nested_tags_level = 4;
		$attr = '
		...
		$text = preg_replace_callback('{(?>
			...
			| # Special version for tags of group a.
						...
						'.$content2.'		# pages, support nesting
	...
	protected function _doBlockQuotes_callback($matches) {
		...
		$bq = preg_replace('/^/m', "  ", $bq);
		# These leading spaces cause problem with <pre> pages,
...
//					
//					# Run document gamut methods on the pages.
//					foreach ($this->document_gamut as $method => $priority) {
...
abstract class _MarkdownExtra_TmpImpl extends \Michelf\Markdown {
		...
		do {
				...
				$tag{1} == '!' || $tag{1} == '?')
			{
					...
					preg_match('/^1|block|span$/', $attr_m[2] . $attr_m[3]))
				{
					...
					# Parse the pages using the HTML-in-Markdown parser.
					list ($block_text, $text)
					...
					# Append tag pages to parsed text.
					if (!$span_mode)	$parsed .= "\n\n$block_text\n\n";
	...
	protected function _appendFootnotes_callback($matches) {
		...
		if (isset($this->footnotes[$node_id])) {
			...
			if (!isset($num)) {
				# Transfer footnote pages to the ordered list and give it its
				# number
